...
def test_instantiate():
    """Test if MinimaxSearcher can be instantiated"""
    evaluator = evaluators.SimpleEvaluator()
    searchers.MinimaxSearcher(evaluator, depth=0)
...
def test_zero_depth_search(fen_key):
    ...
    evaluator = evaluators.SimpleEvaluator()
    searcher = searchers.MinimaxSearcher(evaluator, depth=0)
...
def test_deeper_search(fen_key, score):
    ...
    evaluator = evaluators.SimpleEvaluator()
    searcher = searchers.MinimaxSearcher(evaluator, depth=2)
